	...
	private function compare_versions() {
		...
		if ( version_compare( $wp_version, $this->wp_min, '<' ) ) {
			...
			$this->notices[] = sprintf(
				...
				__( "<strong>%1\$s</strong> requires at least <code>WordPress %2\$s</code>.\nPlease update WordPress to activate the plugin.", 'simple-contact-form' ),
				$this->plugin,
				$this->wp_min
			);
		...
		if ( version_compare( PHP_VERSION, $this->php_min, '<' ) ) {
			...
			$this->notices[] = sprintf(
				...
				__( "<strong>%1\$s</strong> requires at least <code>PHP %2\$s</code>.\nPlease contact your hosting provider to upgrade your PHP version.", 'simple-contact-form' ),
				$this->plugin,
				$this->php_min
			);
...
return new SCF_Compatibility( '4.7.0', '5.4.0', SCF_NAME, SCF_BASE );
